...
(function (LW) {
  ...
  class Renderer {
    ...
    drawLabels(c, z, lod) {
      ...
      for (const s of w.settlements.values()) { if (s.abandonedTick) continue; const p = this.worldToScreen(s.x + 0.5, s.y - 1.2); if (p.x < -100 || p.x > c.canvas.width + 100 || p.y < -50 || p.y > c.canvas.height + 50) continue; const label = `${s.name}`; const sub = `${s.tier} · ${s.population}`; c.fillStyle = 'rgba(0,0,0,0.55)'; const tw = Math.max(c.measureText(label).width, c.measureText(sub).width) + 14; c.fillRect(p.x - tw / 2, p.y - 26, tw, 30); c.fillStyle = '#f1e5c4'; c.fillText(label, p.x, p.y - 13); c.fillStyle = '#bdb090'; c.font = `${Math.max(9, Math.min(12, z * 0.8))}px "Segoe UI", system-ui, sans-serif`; c.fillText(sub, p.x, p.y - 1); c.font = `${Math.max(11, Math.min(16, z))}px "Segoe UI", system-ui, sans-serif`; }
